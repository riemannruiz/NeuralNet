...
def Levenberg_Marquart_training_(X, Y, params, max_iter):
    ...
    model.d_b = [np.array([]) for _ in range(n_capas)]
    model.d_W = [np.array([]) for _ in range(n_capas)]
    ...
    while iter_ <= max_iter:
        ...
        for s in range(Y.shape[1]):
            model.inputs[-1] = inputs_end[:, s].reshape(-1,1)
            model.hidden_out[-1] = hidden_out[:, s].reshape(-1,1)
            model.phi_[-1] = phi_out[:, s].reshape(-1,1)
            params.W[-1] = params_.W[-1][s, :].reshape(1, -1)
            params.b[-1] = params_.b[-1][s, :].reshape(1, -1)

            Jac[:, s * n_samples:(s+1) * n_samples] = Jacobian_k_opt_v2(Y[:, s].reshape(-1,1), model, params, s)
        ...
        while J_loss_ >= model.loss[iter_]:
            ...
            for layer in range(n_capas, 0, -1):
                ...
                Wtot[params.indx_W_Jac[layer - 1], 0] = (Wtmp.T).flatten()
            dW = -np.linalg.inv(Jac @ Jac.T + mu * np.eye(params.nw_total)) @ (Jac @ (E_Jac.T).flatten().reshape(-1,1) + lambda_ * Wtot)
            ...
            for k in range(n_capas-1, -1, -1):
                ...
                d_temp = np.reshape(dW[params.indx_W_Jac[k]], (c+1, f)).T
                model.d_b[k] = d_temp[:, 0].reshape(-1,1)  # Guardar el delta_b de la capa
                model.d_W[k] = d_temp[:, 1:]  # Guardar el delta_W de la capa
                params.b[k] = params.b[k] + model.d_b[k]  # Actualizacion de bias
                params.W[k] = params.W[k] + model.d_W[k]  # Actualizacion de pesos

            # Calculo de la nueva funcion de costo


            Y_hat_,_ = neural_model(model.inputs[0], params)
            J_loss_,_,_,_ = params.loss(Y, Y_hat_, params)
